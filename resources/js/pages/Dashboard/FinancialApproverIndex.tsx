...
export default function FinancialApproverDashboard({
    approverRoleName,
    approverStats,
    recentPending,
    recentDecisions,
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
                    <div className="space-y-1">
                        <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-100">
                            <ShieldCheck size={12} /> {approverRoleName}
                        </div>
                        <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">Team-Based Approvals Control</h1>
                        <p className="text-xs text-slate-500">
                            Aggregate metrics and decision workflows for team-based loan sheets.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3">
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                            ...
                            Overall Territory Status (All Zones)
                        </h3>
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-white rounded-xl border border-slate-200/60 p-4 shadow-sm hover:shadow transition-shadow">
                ...
                <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-slate-200/60 bg-slate-50/50 flex items-center justify-between">
                        <h2 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                            ...
                            Zone Wise Items Status Breakdown ({periodLabel})
                        </h2>
                    </div>
                                ...
                                {breakdown.length === 0 ? (
                                    <tr>
                                        <td colSpan={6} className="py-8 text-center text-slate-400 font-medium">
                                            No zone data recorded in this period.
                                        </td>
                                    </tr>
                                ) : (
                                    breakdown.map((row) => (
                                        <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="py-3 px-5 font-semibold text-slate-900">
                                                <Link
                                                    href={`/team-based-approvals/for-approver?zone_id=${row.id}&date_from=${fromDate || ''}&date_to=${toDate || ''}`}
