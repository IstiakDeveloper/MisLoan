...
class HeadOfficeTeamBasedApprovalController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $approvals = $query
            ...
            ->paginate($perPage)
            ->withQueryString()
            ->through(fn (TeamBasedApprovalItem $item) => $this->formatHeadOfficeIndexItem($item));
    ...
    public function exportItems(Request $request)
    {
        ...
        $rows = $query
            ->orderBy('id', 'desc')
            ->get()
            ->map(fn (TeamBasedApprovalItem $item) => $this->formatHeadOfficeIndexItem($item))
            ->values();
     ...
     */
    private function formatHeadOfficeIndexItem(TeamBasedApprovalItem $item): array
    {
        ...
        DB::transaction(function () use ($validated, $teamBasedApproval) {
            ...
            foreach ($items as $index => $item) {
                ...
                $itemUpdate = [
                    ...
                    'repaid_loan_amount' => TeamBasedApprovalItem::asWholeNumber($row['repaid_loan_amount'] ?? null),
                    'repaid_installment_no' => $row['repaid_installment_no'] ?? null,
                    'other_institution_loan_amount' => $row['other_institution_loan_amount'] ?? null,
                    'proposed_loan_amount' => TeamBasedApprovalItem::asWholeNumber($row['proposed_loan_amount'] ?? null),
    ...
    public function updateItem(Request $request, TeamBasedApprovalItem $item)
    {
        ...
        DB::transaction(function () use ($validated, $item) {
            $item->update([
                ...
                'repaid_loan_amount' => TeamBasedApprovalItem::asWholeNumber($validated['repaid_loan_amount'] ?? null),
                ...
                'proposed_loan_amount' => TeamBasedApprovalItem::asWholeNumber($validated['proposed_loan_amount'] ?? null),
    ...
    public function report(Request $request)
    {
        ...
        $rows = $rowsQuery
            ->select([
                ...
                DB::raw("SUM(CASE WHEN LOWER(r.status) = 'waiting' THEN 1 ELSE 0 END) as waiting_count"),
        ...
        $zoneReports = $zones->map(function (Zone $zone) use ($rows) {
            ...
            return [
                ...
                'waiting' => (int) $zoneRows->sum('waiting_count'),
        ...
        $grandTotals = [
            ...
            'waiting' => (int) collect($zoneReports)->sum('waiting'),
