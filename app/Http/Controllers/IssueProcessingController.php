...
class IssueProcessingController extends Controller
{
    ...
    public function __construct(LoanIssueDetectionService $loanIssueService)
    {
        $this->loanIssueService = $loanIssueService;
    }

    /**
     * Main Dashboard - Show Loan Members with Status
     */
    public function index(Request $request)
    {
        $selectedDate = $request->input('date', now()->toDateString());
        $searchMember = $request->input('search', null);
        $applicationType = $request->input('type', 'loan');

        // Get loan members submitted on selected date
        ...
        $membersData = $members->map(function ($member) {
            return [
                ...
                'application_no' => $member->loanApplication->application_no,
                'branch_name' => $member->loanApplication->branch?->name,
            ];
        });
    ...
    private function sendIssueNotificationEmail($type, $id, $issues)
    {
        ...
        foreach ($issues as $memberIssues) {
            foreach ($memberIssues as $issue) {
                $memberName = $issue->loanMember?->member_name ?? 'Unknown';
    ...
    public function submitBatch(Request $request)
    {
        ...
        try {
            ...
            // Auto-approve loan members without issues on this date
    ...
    public function getUpdatedMembers(Request $request)
    {
        ...
        $membersData = $members->map(function ($member) {
            ...
            return [
                ...
                'application_no' => $member->loanApplication->application_no,
                'branch_name' => $member->loanApplication->branch?->name,
                'loan_application_id' => $member->loanApplication?->id,
    ...
    public function getUpdatedIssues(Request $request, $id)
    {
        ...
        foreach ($issues as $issue) {
            ...
            if (!isset($issuesByMember[$memberId])) {
                $memberModel = LoanMember::find($memberId);
